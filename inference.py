#!/usr/bin/env python3
# -*- coding: utf-8 -*-
from tqdm import tqdm
import pandas as pd

import numpy as np

import torch
from torch import ones, zeros, tensor

from torch.distributions import constraints, biject_to

import pyro

from pyro.distributions.util import broadcast_shape, sum_rightmost

import pyro.distributions as dist
from pyro import sample, param, plate, markov, poutine, clear_param_store, get_param_store, module
from pyro.infer import SVI, TraceEnum_ELBO

from pyro.infer.discrete import TraceEnumSample_ELBO, infer_discrete

from pyro.optim import Adam
from pyro.ops.indexing import Vindex


class Inferrer(object):
    def __init__(self,
                 agent,
                 stimuli,
                 responses,
                 mask):

        self.agent = agent
        self.nsub, self.nblk = responses.shape[:2]

        self.responses = responses
        self.mask = mask
        self.N = mask.sum(dim=0)

        self.depth_transition = zeros(2, 3, 2, 3)
        self.depth_transition[0, :, 0] = tensor([1., 0., 0.])
        self.depth_transition[0, :, 1] = tensor([.5, .5, 0.])
        self.depth_transition[1] = tensor([1., 0., 0.])

        self.states = stimuli['states']
        self.configs = stimuli['configs']
        self.conditions = stimuli['conditions']

    def model_static(self):
        """Assume static prior over planning depth per condition.
        """
        agent = self.agent
        np = agent.np  # number of parameters

        nblk = self.nblk  # number of mini-blocks
        nsub = self.nsub  # number of subjects

        # define hyper priors over model parameters.

        # define prior uncertainty over model parameters and subjects
        a = param('a', 2*ones(np), constraint=constraints.positive)
        r = param('r', 2*ones(np), constraint=constraints.positive)
        tau = sample('tau', dist.Gamma(a, r/a).to_event(1))

        sig = 1./torch.sqrt(tau) # transformation to finally get normal-inverse-gamma für mu, see below
        # define prior mean over model parameters
        m = param('m', zeros(np))
        s = param('s', ones(np), constraint=constraints.positive)
        mu = sample("mu", dist.Normal(m, s*sig).to_event(1)) # see # https://en.wikipedia.org/wiki/Normal-inverse-gamma_distribution : mu effectively follows normal-inverse-gamma with parameters aplha=a, beta=a/r, mü=m, lamba=1/s²

        alphas_c1 = param("alphas_c1", ones(2), constraint=constraints.positive)
        alphas_c2 = param("alphas_c2", ones(3), constraint=constraints.positive)

        with plate('subjects', nsub):
            # locs.shape : [num_particles, num_subjects, 3]
            locs = sample("locs", dist.Normal(mu, sig).to_event(1))
            # define priors over planning depth
            probs_c1 = sample("probs_c1", dist.Dirichlet(alphas_c1)) # prior over planning depths if remaining steps = 2 (i.e. for a1 in 3-step miniblock and a0 in 2-step miniblock)
            probs_c2 = sample("probs_c2", dist.Dirichlet(alphas_c2)) # prior over planning depths if remaining steps = 3 (i.e. for a0 in 3-step miniblock)

        agent.set_parameters(locs)

        shape = agent.batch_shape

        tmp = zeros(shape + (3,))
        tmp[..., :2] = probs_c1

        priors = torch.stack([tmp, probs_c2], -2)

        for b in markov(range(nblk)):
            conditions = self.conditions[..., b]
            states = self.states[:, b]
            responses = self.responses[:, b]
            max_trials = conditions[-1]

            tm = self.depth_transition[:, :, max_trials - 2]
            for t in markov(range(3)):

                if t == 0:
                    res = None
                    probs = priors[..., range(nsub), max_trials - 2, :]
                else:
                    res = responses[:, t-1]
                    probs = tm[t-1, -1]

                agent.update_beliefs(b, t, states[:, t], conditions, res)
                agent.plan_actions(b, t)

                valid = self.mask[:, b, t]
                N = self.N[b, t]
                res = responses[valid, t]

                logits = agent.logits[-1][..., valid, :]
                probs = probs[..., valid, :]

                if t == 2:
                    agent.update_beliefs(b, t + 1, states[:, t + 1], conditions, responses[:, t])

                if N > 0:
                    with plate('responses_{}_{}'.format(b, t), N):
                        d = sample('d_{}_{}'.format(b, t),
                                   dist.Categorical(probs),
                                   infer={"enumerate": "parallel"})

                        sample('obs_{}_{}'.format(b, t),
                               dist.Bernoulli(logits=Vindex(logits)[..., d]),
                               obs=res)

    def model_dynamic(self):
        """Assume dynamic transition between planning depths between mini-blocks.
        """
        agent = self.agent
        np = agent.np  # number of parameters

        nblk = self.nblk  # number of mini-blocks
        nsub = self.nsub  # number of subjects

        # define hyper priors over model parameters.
        # prior uncertanty
        a = param('a', 2*ones(nsub, np), constraint=constraints.positive)
        r = param('r', 2*ones(nsub, np), constraint=constraints.positive)

        # prior mean
        m = param('m', zeros(np))
        lam = param('lam', ones(nsub, 1), constraint=constraints.positive)

        nuc1 = param('nu_c1', ones(1), constraint=constraints.positive)
        nuc2 = param('nu_c2', ones(1), constraint=constraints.positive)

        with plate('subjects', nsub):
            tau = sample('tau', dist.Gamma(a, r/a).to_event(1))
            mu = sample("mu", dist.Normal(m, 1/torch.sqrt(lam*tau)).to_event(1))

            locs = sample("locs", dist.Normal(mu, 1/torch.sqrt(tau)).to_event(1))
            # define priors over planning depth
            probsc1 = sample("probs_c1", dist.Dirichlet(ones(2)*nuc1))
            probsc2 = sample("probs_c2", dist.Dirichlet(ones(3)*nuc2))

        with plate('depth', 3):
            with plate('subs', nsub):
                # define priors over planning depth transition matrix
                rho1 = sample("rho_c1", dist.Dirichlet(ones(2)))
                rho2 = sample("rho_c2", dist.Dirichlet(ones(3)))

        agent.set_parameters(locs)

        tmp1 = zeros(nsub, 3)
        tmp1[:, :2] = probsc1

        priors = torch.stack([tmp1, probsc2])

        tmp2 = zeros(nsub, 3, 3)
        tmp2[..., :2] = rho1

        rho = torch.stack([tmp2, rho2])

        x = tensor([0]*nsub)
        for b in markov(range(nblk)):
            conditions = self.conditions[..., b]
            states = self.states[:, b]
            responses = self.responses[:, b]
            max_trials = conditions[-1]

            tm = self.depth_transition[:, :, max_trials - 2]
            N = self.N[b, 0]

            if b == 0:
                probs = priors[max_trials - 2, range(nsub)].unsqueeze(0)
            else:
                probs = rho[max_trials - 2, range(nsub)].transpose(dim1=0, dim0=1)

            agent.update_beliefs(b, 0, states[:, 0], conditions, None)
            agent.plan_actions(b, 0)

            res = responses[:, 0]
            logits = agent.logits[-1]

            with plate('responses_{}_0'.format(b), N) as ind:
                if b == 0:
                    d = sample('d_{}_{}'.format(b, 0),
                               dist.Categorical(probs[x, ind]),
                               infer={"enumerate": "parallel"})
                else:
                    d = sample('d_{}_{}'.format(b, 0),
                               dist.Categorical(probs[d, ind]),
                               infer={"enumerate": "parallel"})

                sample('obs_{}_0'.format(b),
                       dist.Bernoulli(logits=logits[d, ind]),
                       obs=res)

            for t in markov(range(1, 3)):
                valid = self.mask[:, b, t]
                N = self.N[b, t]

                res = responses[:, t-1]
                probs = tm[t-1, -1][valid]

                agent.update_beliefs(b, t, states[:, t], conditions, res)
                agent.plan_actions(b, t)

                res = responses[valid, t]

                logits = agent.logits[-1][:, valid]

                if t == 1:
                    with plate('responses_{}_{}'.format(b, t), N):
#                        d1 = sample('d_{}_{}'.format(b, t),
#                           dist.Categorical(probs),
#                           infer={"enumerate": "parallel"})

                        sample('obs_{}_{}'.format(b, t),
                           dist.Categorical(logits=logits[0]),
                           obs=res)
                else:
                    if N > 0:
                        with plate('responses_{}_{}'.format(b, t), N):
                            sample('obs_{}_{}'.format(b, t),
                                   dist.Categorical(logits=logits[0]),
                                   obs=res)

    def guide_static(self):

        npar = self.agent.np  # number of parameters
        # nblk = self.nblk
        nsub = self.nsub  # number of subjects

        m_hyp = param('m_hyp', zeros(2*npar))
        st_hyp = param('scale_tril_hyp',
                       torch.eye(2*npar),
                       constraint=constraints.lower_cholesky)
        hyp = sample('hyp', dist.MultivariateNormal(m_hyp, scale_tril=st_hyp),
                     infer={'is_auxiliary': True})

        unc_mu = hyp[..., :npar]
        unc_tau = hyp[..., npar:]

        trns_tau = biject_to(constraints.positive)

        c_tau = trns_tau(unc_tau)

        ld_tau = trns_tau.inv.log_abs_det_jacobian(c_tau, unc_tau)
        ld_tau = sum_rightmost(ld_tau, ld_tau.dim() - c_tau.dim() + 1)

        mu = sample("mu", dist.Delta(unc_mu, event_dim=1))
        tau = sample("tau", dist.Delta(c_tau, log_density=ld_tau, event_dim=1))

        m_locs = param('m_locs', zeros(nsub, npar))
        st_locs = param('s_locs', torch.eye(npar).repeat(nsub, 1, 1),
                        constraint=constraints.lower_cholesky)

        alpha1 = param('alpha1', ones(nsub, 2), constraint=constraints.positive)
        alpha2 = param('alpha2', ones(nsub, 3), constraint=constraints.positive)

        with plate('subjects', nsub):
            # print('m_loc[0,2]: ' + str(m_locs[0,2])) # TODO: remove (just for debugging)
            locs = sample("locs", dist.MultivariateNormal(m_locs, scale_tril=st_locs))
            probsc1 = sample("probs_c1", dist.Dirichlet(alpha1))
            probsc2 = sample("probs_c2", dist.Dirichlet(alpha2))

        return {'mu': mu, 'tau': tau, 'locs': locs, 'pc1': probsc1, 'pc2': probsc2}

    def guide_dynamic(self):

        npar = self.agent.np  # number of parameters
        nblk = self.nblk  # number of blocks
        nsub = self.nsub  # number of subjects

        m_hyp = param('m_hyp', zeros(nsub, 2*npar))
        st_hyp = param('scale_tril_hyp',
                       torch.eye(2*npar).repeat(nsub, 1, 1),
                       constraint=constraints.lower_cholesky)

        m_locs = param('m_locs', zeros(nsub, npar))
        st_locs = param('s_locs', torch.eye(npar).repeat(nsub, 1, 1),
                        constraint=constraints.lower_cholesky)

        alpha1 = param('alpha1', ones(nsub, 2), constraint=constraints.positive)
        alpha2 = param('alpha2', ones(nsub, 3), constraint=constraints.positive)

        with plate('subjects', nsub):
            hyp = sample('hyp', dist.MultivariateNormal(m_hyp, scale_tril=st_hyp),
                         infer={'is_auxiliary': True})

            unc_mu = hyp[..., :npar]
            unc_tau = hyp[..., npar:]

            trns_tau = biject_to(constraints.positive)

            c_tau = trns_tau(unc_tau)

            ld_tau = trns_tau.inv.log_abs_det_jacobian(c_tau, unc_tau)
            ld_tau = sum_rightmost(ld_tau, ld_tau.dim() - c_tau.dim() + 1)

            mu = sample("mu", dist.Delta(unc_mu, event_dim=1))
            tau = sample("tau", dist.Delta(c_tau, log_density=ld_tau, event_dim=1))

            locs = sample("locs", dist.MultivariateNormal(m_locs, scale_tril=st_locs))
            probsc1 = sample("probs_c1", dist.Dirichlet(alpha1))
            probsc2 = sample("probs_c2", dist.Dirichlet(alpha2))

        beta1 = param('beta1', ones(nsub, 3, 2), constraint=constraints.positive)
        beta2 = param('beta2', ones(nsub, 3, 3), constraint=constraints.positive)

        with plate('depth', 3):
            with plate('subs', nsub):
                # define priors over planning depth transition matrix
                sample("rho_c1", dist.Dirichlet(beta1))
                sample("rho_c2", dist.Dirichlet(beta2))

        return {'mu': mu, 'tau': tau, 'locs': locs, 'pc1': probsc1, 'pc2': probsc2}

    def fit(self,
            num_iterations=100,
            num_particles=100,
            vectorize_particles=True,
            optim_kwargs={'lr': .1},
            parametrisation='static'):

        clear_param_store()

        if parametrisation == 'static':
            self.model = self.model_static
            self.guide = self.guide_static
            self.mpn = 1
        elif parametrisation == 'dynamic':
            self.model = self.model_dynamic
            self.guide = self.guide_dynamic
            self.mpn = 2

        # elbo = TraceEnumSample_ELBO(num_particles=num_particles,
        #                             vectorize_particles=vectorize_particles,
        #                             max_plate_nesting=self.mpn)

        elbo = TraceEnum_ELBO(num_particles=num_particles,
                              vectorize_particles=vectorize_particles,
                              max_plate_nesting=self.mpn)

        svi = SVI(model=self.model,
                  guide=self.guide,
                  optim=Adam(optim_kwargs),
                  loss=elbo)

        loss = []
        pbar = tqdm(range(num_iterations), position=0)
        for step in pbar:
            loss.append(svi.step())
            
            # TODO: add logg of values of needed here:
            #print('  m_locs for param 3 (k), sub1: ' + str(param('m_locs').data.numpy()[0,2]) + ' | sub2: ' + str(param('m_locs').data.numpy()[1,2]))
            
            pbar.set_description("Mean ELBO %6.2f" % torch.Tensor(loss[-20:]).mean())

        param_store = get_param_store()
        parameters = {}
        for name in param_store.get_all_param_names():
            parameters[name] = param(name)

        self.parameters = parameters
        self.loss = loss
        self.elbo = elbo
        self.svi  = svi

    def sample_from_posterior(self, labels, n_samples=1000):

        import numpy as np
        nsub = self.nsub
        npars = self.agent.np
        assert npars == len(labels)

        keys = ['mu', 'tau', 'locs']

        trans_pars = np.zeros((n_samples, nsub, npars))

        mean_global = np.zeros((n_samples, npars))
        tau_global = np.zeros((n_samples, npars))

        for i in range(n_samples):
            sample = self.guide()
            for key in keys:
                sample.setdefault(key, ones(1))

            mu = sample['mu']
            tau = sample['tau']
            pars = sample['locs']

            trans_pars[i] = pars.detach().numpy()

            mean_global[i] = mu.detach().numpy()
            tau_global[i] = tau.detach().numpy()

        order = (self.conditions[-1, :, 0] == 3) + 1
        order_label = np.tile(order, (n_samples, 1)).reshape(-1)
        subject_label = np.tile(range(1, nsub+1), (n_samples, 1)).reshape(-1)
        tp_df = pd.DataFrame(data=trans_pars.reshape(-1, npars), columns=labels)
        tp_df['subject'] = subject_label
        tp_df['order'] = order_label

        mg_df = pd.DataFrame(data=mean_global, columns=labels)
        tg_df = pd.DataFrame(data=tau_global, columns=labels)

        return tp_df, mg_df, tg_df

    def sample_posterior_marginal(self, n_samples=100):

        elbo = TraceEnum_ELBO(max_plate_nesting=self.mpn)
        post_depth_samples = {}

        pbar = tqdm(range(n_samples), position=0)

        for n in pbar:
            pbar.set_description("Sample posterior depth")
            # get marginal posterior over planning depths
            
            post_depth = elbo.compute_marginals(self.model, self.guide) #TODO: ??? Warum muss man hier noch einmal mit ELBO arbeiten? ??? Warum kommen die posterior depths pro minib-block nicht vernünftig raus, wenn man das model einfach mit pyro.poutine.trace(model).get_trace() durchlaufen lässt?
            
            for name in post_depth.keys():
                post_depth_samples.setdefault(name, [])
                post_depth_samples[name].append(post_depth[name].probs.detach().clone())

        for name in post_depth_samples.keys():
            post_depth_samples[name] = torch.stack(post_depth_samples[name]).numpy()

        return post_depth_samples

#    def calc_WAIC(self):
#        waic = self.svi.information_criterion()  # returns empty dict
#        return waic

    def calc_BIC(self, m_prob, IDs, n_actions_considered = 1):
        # Evaluate (log-)likelihood of actual choices, given the inferred model parameters:    
        # TODO: n_actions_considered > 1 würde noch nicht korrekt funktionieren...

        agent = self.agent
        conditions = self.conditions

        n_miniblocks = self.nblk  # number of mini-blocks
        n_subj = self.nsub        # number of subjects
        m_param_count = agent.np  # number of parameters

        # initialize
        nll_firstaction_mean        = np.nan * np.ones([n_subj, n_miniblocks])    
        nll_firstaction_depths      = np.nan * np.ones([n_subj, n_miniblocks, 3])

        nll_hinoise_all             = np.nan * np.ones([n_subj, 3])
        nll_lonoise_all             = np.nan * np.ones([n_subj, 3])
        nll_all                     = np.nan * np.ones([n_subj, 3])

        nll_hinoise_mean            = np.nan * np.ones([n_subj])
        nll_lonoise_mean            = np.nan * np.ones([n_subj])
        nll_mean                    = np.nan * np.ones([n_subj])
    	
        pseudo_rsquare_mean         = np.nan * np.ones([n_subj])
        pseudo_rsquare_hinoise_mean = np.nan * np.ones([n_subj])
        pseudo_rsquare_lonoise_mean = np.nan * np.ones([n_subj])

        BIC_mean                    = np.nan * np.ones([n_subj])
        BIC_hinoise                 = np.nan * np.ones([n_subj])
        BIC_lonoise                 = np.nan * np.ones([n_subj])

        for i_subj in range(n_subj):
            for i_mblk in range(n_miniblocks):
                for i_action in range(n_actions_considered): # Consider only first action; use range(3) to consider all action steps
                    logits_depths = agent.logits[3*i_mblk + i_action].detach().numpy() 
                
                    resp = self.responses[i_subj][i_mblk].numpy()[i_action] 
                    
                    #The logits haven't been transformed into probabilities yet.--> softmax function for choice probabilities
                    p_jump_depths = 1.0 / (1 + np.exp(-logits_depths[i_subj])) 
                        
                    for i,p in enumerate(p_jump_depths):
                        if p == 0:
                            p_jump_depths[i] = 1e-7   # for numerical stability
                        elif p == 1:
                            p_jump_depths[i] = 1 - 1e-7  # for numerical stability
                    
                    p_move_depths = 1 - p_jump_depths
                    
                    if resp==1:
                        nll = -np.log(p_jump_depths) # obtain the negative log-likelihood (nll), for numerical reasons
                    elif resp==0:
                        nll = -np.log(p_move_depths)        
                        
                    nll_firstaction_depths[i_subj, i_mblk, :] = nll              
                    nll_firstaction_mean[i_subj, i_mblk] = np.matmul(nll_firstaction_depths[i_subj, i_mblk, :], 
                                                                    m_prob[0][i_mblk, i_subj, :])                 # TODO: is this correct? this way, m_probs are not in log-space. shouldn't we calculate weighted sum of p_move or p_jump first and then take the log of that?
                    
            # TODO: wofür werden die nll_..._all arrays überhaupt gebraucht? -> sind einzeln pro mini-block, aber wurden nicht weiter benutzt
            nll_hinoise_all[i_subj, :] = np.matmul(nll_firstaction_depths[i_subj, :, :].transpose(), 
                                                conditions[0][i_subj, :].numpy()) # Sum of NLL for high noise (noise==1) 
            nll_lonoise_all[i_subj, :] = np.matmul(nll_firstaction_depths[i_subj, :, :].transpose(), 
                                                1 - conditions[0][i_subj, :].numpy()) # Sum of NLL for low noise (noise==0)
            nll_all[i_subj, :] = nll_hinoise_all[i_subj, :] + nll_lonoise_all[i_subj, :]

            nll_hinoise_mean[i_subj] = np.matmul(nll_firstaction_mean[i_subj, :].transpose(), conditions[0][i_subj, :].numpy()) # Include first 20 warmup miniblocks
            nll_lonoise_mean[i_subj] = np.matmul(nll_firstaction_mean[i_subj, :].transpose(), 1 - conditions[0][i_subj, :].numpy())
            
            nll_mean[i_subj] = nll_hinoise_mean[i_subj] + nll_lonoise_mean[i_subj]

            nll_random      = -np.log(0.5)* 140 * n_actions_considered
            nll_random_half = -np.log(0.5)* 70  * n_actions_considered
            
            pseudo_rsquare_mean[i_subj] = 1 - (nll_mean[i_subj] / nll_random)
            pseudo_rsquare_hinoise_mean[i_subj] = 1 - (nll_hinoise_mean[i_subj] / nll_random_half)
            pseudo_rsquare_lonoise_mean[i_subj] = 1 - (nll_lonoise_mean[i_subj] / nll_random_half)

            # TODO: wofür wird das BIC_mean gebraucht? Im paper wird dann der average über hinoise und lonoise berichtet, der ist aber anders denn BIC_mean ist hier grob die Summe aus den beiden?
            BIC_mean[i_subj]    = 2*nll_mean[i_subj]         + m_param_count*np.log(140 * n_actions_considered) # !Achtung BIC wächst mit mehr Daten (mehr trials), daher ist BIC_mean hier so groß
            BIC_hinoise[i_subj] = 2*nll_hinoise_mean[i_subj] + m_param_count*np.log( 70 * n_actions_considered)
            BIC_lonoise[i_subj] = 2*nll_lonoise_mean[i_subj] + m_param_count*np.log( 70 * n_actions_considered)


        fitdict = {}
        fitdict['nll_1staction_mean'] = nll_mean
        fitdict['nll_1staction_hinoise_mean'] = nll_hinoise_mean
        fitdict['nll_1staction_lonoise_mean'] = nll_lonoise_mean    
        fitdict['BIC_mean'] = BIC_mean
        fitdict['BIC_hinoise_mean'] = BIC_hinoise
        fitdict['BIC_lonoise_mean'] = BIC_lonoise
        fitdict['pseudo_Rsquare_1staction_mean'] = pseudo_rsquare_mean
        fitdict['pseudo_Rsquare_1staction_hinoise_mean'] = pseudo_rsquare_hinoise_mean
        fitdict['pseudo_Rsquare_1staction_lonoise_mean'] = pseudo_rsquare_lonoise_mean   
        fitdict['ID'] = IDs

        fit_df = pd.DataFrame(data=fitdict)
                    
        return fit_df
    



    def calc_WAIC_and_DIC(self, IDs, num_samples=100, variance_correction = True, n_actions_considered = 1):  
        # variance_correction e.g. for tensor.var() function is True in waic() function in pyro.ops.stats module but is False in arviz.waic() function
        # TODO: n_actions_considered > 1 würde noch nicht korrekt funktionieren... 
        # TODO: directly use the posterior samples obtained from sample_from_posterior and sample_from_posterior_marginal function
        '''
        WAIC: Gelman, Andrew; Carlin, John B.; Stern, Hal S.; Rubin, Donald B. (2004). 
        Bayesian Data Analysis: Second Edition
        (WAIC as deviance, i.e. smaller values are better)
        '''
        num_obs = self.nblk  # only use observations of first action which equals number of miniblocks
        noise_condition = self.conditions[0,0,:] # [(0-noise, 1-steps), sub, mb] -> get noise condition of task from first subject 
        
        '''
        Likelihood function
        '''    
        like = torch.zeros(num_samples, num_obs, self.nsub)
        like[:] = torch.nan
        loglike = torch.zeros(num_samples, num_obs, self.nsub)
        loglike[:] = torch.nan
        
        pbar = tqdm(range(num_samples), position=0)
        for i in pbar:   
            pbar.set_description("Sample posterior predictions")               
            # sample locs from approx. posterior (guide)
            guide_sample = self.guide()
            # sample posterior depths per miniblock via marginal
            elbo = TraceEnum_ELBO(max_plate_nesting=self.mpn)
            post_depth = elbo.compute_marginals(self.model, self.guide)
            # simulate agent with sampled locs
            agent = self.agent
            agent.set_parameters(guide_sample['locs'].detach())
            
            nblk = agent.nmb
            logits_b_t_s_d    = torch.zeros(nblk, 3, self.nsub, 3)
            logits_b_t_s_d[:] = torch.nan

            for b in range(nblk):
                conditions = self.conditions[..., b]
                states = self.states[:, b]
                responses = self.responses[:, b]
                max_trials = conditions[-1]

                for t in range(3):

                    if t == 0:
                        res = None
                    else:
                        res = responses[:, t-1]

                    agent.update_beliefs(b, t, states[:, t], conditions, res)
                    agent.plan_actions(b, t)

                    valid = self.mask[:, b, t]
                    res = responses[valid, t]

                    logits = agent.logits[-1][..., valid, :]
                    logits_b_t_s_d[b,t,...] = logits

                    if t == 2:
                        agent.update_beliefs(b, t + 1, states[:, t + 1], conditions, responses[:, t])

                # extract logits of first action
                logits_a0_s_d = logits_b_t_s_d[b,0,...]
                probs_a0_s_d  = torch.sigmoid(logits_a0_s_d)
                
                # get weighted average over depths of choice probabilites
                probs = torch.matmul(probs_a0_s_d, post_depth[f'd_{b}_0'].probs.T).diag()   # p(a0=jump)   (weighted average over depths)
                probs = torch.clamp(probs, min=1e-7, max=1-1e-7)                            # clamp min and max to prevent 0 and 1 values which can later cause inf or -inf during log()
                counter_probs = 1 - probs                                                   # p(a0=move)
                choices = self.responses[:,b,0].to(torch.bool)                              # actual responses of participants (data) -> here assumed to be binary (bool) for later use in tensor.where()

                # get likelihood according to choices/responses
                like[i, b, ...] = probs.where(choices,counter_probs).detach()
                loglike[i, b, ...] = torch.log(probs.where(choices,counter_probs)).detach()
        
        '''
        Effective number of parameters (or better: overfitting penalty or overfitting risk)
        '''
        loglike_2D = torch.zeros((num_samples, self.nsub*num_obs)) # TODO: sub and obs are pooled into one dimension here, is this accurate?
        like_2D = torch.zeros((num_samples, self.nsub*num_obs))
        for i in range(num_samples):
            loglike_2D[i, :] = torch.flatten(loglike[i, :, :])
            like_2D[i,:] =  torch.flatten(like[i, :, :])
            
        pwaic_vec = loglike_2D.var(axis=0, correction = 1)   # variance of samples for each obs and sbj (Sascha had 'correction=0' everwhere, why?! -> changed all to 1 to get unbiased variances)
        pwaic = pwaic_vec.nansum()  # sum of variances of samples
        
        individual_pwaic_vec = loglike.var(axis=0, correction = 1)
        individual_pwaic = individual_pwaic_vec.nansum(axis=0)
        
        pointwise_pwaic = loglike.var(axis=0, correction = 1)
        '''
        WAIC
        '''
        lppd_vec = torch.log(like_2D.mean(axis=0))
        lppd = lppd_vec.nansum()
        WAIC = -2*(lppd - pwaic)
        
        individual_lppd_vec = torch.log(like.mean(axis=0))
        individual_lppd = individual_lppd_vec.nansum(axis = 0)
        individual_WAIC = -2*(individual_lppd - individual_pwaic)
        
        pointwise_lppd = torch.log(like.mean(axis=0))
        pointwise_WAIC = -2*(pointwise_lppd - pointwise_pwaic)
        
        '''
        Variance of WAIC (not validated. E.g. arviz.waic() just computes waic_se as standard error over pointwise waics)
        '''
        # Var(X+Y) = Var(X) + Var(Y) + 2Cov(X,Y)
        # Var(X-Y) = Var(X+(-Y)) = Var(X) + Var(-Y) + 2Cov(X,-Y) = Var(X) + Var(Y) - 2*Cov(X,Y)
        # Var(aX)  = a²Var(X)
        lppd_var = lppd_vec[~torch.isnan(lppd_vec)].var(correction = 1)
        pwaic_var = pwaic_vec[~torch.isnan(pwaic_vec)].var(correction = 1)
        cov = torch.cov(torch.stack((lppd_vec[((~torch.isnan(lppd_vec)) & (~torch.isnan(pwaic_vec)))], pwaic_vec[((~torch.isnan(lppd_vec)) & (~torch.isnan(pwaic_vec)))])), correction=1)
        assert cov.ndim == 2
        assert cov.shape[0] == 2
        assert cov.shape[1] == 2
        waic_var = 4*(lppd_var + pwaic_var - 2*cov[0,1])
        
        # TODO: get separate waic_var for subject-level and for mini-block level
        
        '''
        DIC (Deviance information criterion) Gelman, Andrew; Carlin, John B.; Stern, Hal S.; Rubin, Donald B. (2004). 
        Bayesian Data Analysis: Second Edition
        Effective number of parameters pD = 2*(log p(y|θ_Bayes) - E_post[log p(y|θ)])
        θ_Bayes : mean of posterior
        E_post[log p(y|θ)] : mean of log p(y|θ) under the posterior of θ
        '''
        
        # initialize tensors for param samples
        num_params = self.agent.np # number of loc params
        loc_samples = torch.zeros((self.nsub, num_params, num_samples))
        pc1_samples = torch.zeros((self.nsub, 2, num_samples))
        pc2_samples = torch.zeros((self.nsub, 3, num_samples))    
        
        for sample in range(num_samples):
            # sample from approx. posterior (guide)
            guide_sample = self.guide()  
            loc_samples[..., sample] = self.guide()['locs'].detach()
            pc1_samples[..., sample] = self.guide()['pc1'].detach()
            pc2_samples[..., sample] = self.guide()['pc2'].detach()
        
        # compute means of sample
        loc_samples_mean = loc_samples.mean(axis=-1)
        pc1_samples_mean = pc1_samples.mean(axis=-1)
        pc2_samples_mean = pc2_samples.mean(axis=-1)
        
        # condition the likelihood (model) on one sample from the approx. posterior
        conditioned_model = pyro.condition(self.model, 
                                        data = {'locs':     loc_samples_mean,
                                                'probs_c1': pc1_samples_mean,
                                                'probs_c2': pc2_samples_mean
                                                })
        
        trace = pyro.poutine.trace(conditioned_model).get_trace()
        
        DIC_loglike = 0
        individual_DIC_loglike = torch.zeros(self.nsub)
        for key, val in trace.nodes.items():
            if key[:4] == 'obs_' and key[-2:] == '_0':   # only use observations of first action
                choices = val['value'].to(torch.bool)    # actual responses of participants (data) -> here assumed to be binary (bool) for later use in tensor.where()
                probs = val['fn'].probs                  # p(a0=1 also jump)
                probs = torch.clamp(probs, min=1e-7, max=1-1e-7)  # clamp min and max to prevent 0 and 1 values which can later cause inf or -inf during log()
                counter_probs =  1 - probs               # p(a0=move)
        
                DIC_loglike += torch.log(probs.where(choices,counter_probs)).sum().detach()
                individual_DIC_loglike += torch.log(probs.where(choices,counter_probs)).detach()
        
        pDIC = 2*(DIC_loglike - loglike.mean(axis=0).nansum())
        DIC = -2*DIC_loglike + 2*pDIC
        
        individual_pDIC = 2*(individual_DIC_loglike - loglike.mean(axis=0).nansum(axis=0))
        individual_DIC = -2*individual_DIC_loglike + 2*individual_pDIC


        # put everything in a dictionary (individual sums over miniblocks)
        fitdict = {}
        fitdict['ID'] = IDs
        fitdict['WAIC']           = individual_WAIC.detach().numpy(force=True)
        fitdict['WAIC_lowNoise']  = pointwise_WAIC[torch.where(noise_condition==0)[0] , :].sum(dim=0).detach().numpy(force=True)
        fitdict['WAIC_highNoise'] = pointwise_WAIC[torch.where(noise_condition==1)[0] , :].sum(dim=0).detach().numpy(force=True)
        fitdict['pWAIC']          = individual_pwaic.detach().numpy(force=True)
        fitdict['DIC']            = individual_DIC.detach().numpy(force=True)
        fitdict['NLL_sampled']    = (-1) * torch.log(like.mean(dim=0)).sum(dim=0).detach().numpy(force=True) # sum over miniblocks of log of mean of likelihood samples
        

        fit_df = pd.DataFrame(data=fitdict)
                    
        return fit_df, pointwise_WAIC, pointwise_pwaic, loglike
    


    
    def calc_fit_measures(self, IDs, m_prob, num_samples=1000, variance_correction = True):
        '''
        Calculate WAIC, DIC, BIC, and pseudo-R^2
        '''
        fit_df, pointwise_WAIC, pointwise_pwaic, loglike = self.calc_WAIC_and_DIC(IDs, num_samples, variance_correction)
        fit_df_BIC = self.calc_BIC(m_prob, IDs)

        # merge both dfs
        fit_df = pd.merge(fit_df, fit_df_BIC, on='ID', how='outer')
        
        return fit_df, pointwise_WAIC, pointwise_pwaic, loglike
